...
public class CharacterCameraFollow : MonoBehaviour 
{
	...
	void HandlePlayerInput()
	{
		...
		if(!CharacterManager.isBusy && !Pause.isPaused && !GameOver.isGameOver)
		{
			...
			if(!invertX)
			{
				...
				mouseX += Input.GetAxis ("RightHorizontal") * mouseSensitivityX;
			}
			else
			{
				mouseX -= Input.GetAxis ("Mouse X") * mouseSensitivityX;
			...
			if(!invertY)
			{
				...
				mouseY -= Input.GetAxis ("RightVertical") * mouseSensitivityY;
			}
			else
			{
				mouseY += Input.GetAxis ("Mouse Y") * mouseSensitivityY;
	...
	void FixingRotation()
	{
		if(transform.localEulerAngles.y > 0f && transform.localEulerAngles.y < 10f ||
		   transform.localEulerAngles.y >= 350f && transform.localEulerAngles.y <= 360f)
		{
			fixingRotation = false;
		}

		if(transform.localEulerAngles.y > 10f && transform.localEulerAngles.y < 180f)
		{
			mouseX -= fixSpeed;
		}

		if(transform.localEulerAngles.y >= 180f && transform.localEulerAngles.y < 350f)
		{
			mouseX += fixSpeed;
		}
	}
