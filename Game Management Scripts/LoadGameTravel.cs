...
public class LoadGameTravel : MonoBehaviour 
{
	...
	void OnLevelWasLoaded(int level)
	{
		...
		else
		{
			...
			if(quickload)
			{
				GameObject.FindGameObjectWithTag ("Adventure Manager").SendMessage ("Quickload", SendMessageOptions.DontRequireReceiver);
			}
			else if(autoload)
			{
				GameObject.FindGameObjectWithTag ("Adventure Manager").SendMessage ("Autoload", SendMessageOptions.DontRequireReceiver);
			}
			else
			{
				GameObject.FindGameObjectWithTag ("Adventure Manager").SendMessage ("Load", saveFileNumber, SendMessageOptions.DontRequireReceiver);
			}
//			print ("It worked");
			Destroy (gameObject);
		}
